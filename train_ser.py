...
os.environ["CUDA_VISIBLE_DEVICES"] = "0, 1, 2, 3"#GPU的显卡位置。现在能用的GPU显卡的号是，1卡和3卡   #下面的代码要从这两个卡中选，选第一个还是第二个0/1 
...
def train(dataset, params, save_label='default'):
    ...
    if torch.cuda.device_count() > 1:  # 检查电脑是否有多块GPU
        ...
        model = nn.DataParallel(model, device_ids=[0,1,2,3])  # #选上面对应的物理位置。将模型对象转变为多GPU并行运算的模型
    model.to(device)
